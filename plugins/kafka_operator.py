...
class KafkaProduceOperator(BaseOperator):
    ...
    def generate_transaction_data(self, row_num):
        ...
        # generate  list of numbers for randomly select in next step
        customer_ids = [f"C{str(i).zfill(5)}" for i in range(1, self.num_records+1)]
        account_ids = [f"A{str(i).zfill(8)}" for i in range(1, self.num_records + 1)]
        ...
        '''
        fields in the transaction fact table, randomly select from list
        '''
    ...
    def execute(self, context: Context) -> Any:
        '''
        serialize each record to json byte, 
        send to kafka
        '''
